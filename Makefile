# Reproducible packaging for com_yscbcalendar: the ZIP bytes depend only on the
...
# claims. The workflow and its rationale are in README.md, section "Building".

SHELL := /bin/sh

EXT_NAME := com_yscbcalendar
MANIFEST := yscbcalendar.xml
RELEASE_NOTES := docs/RELEASE.md
GITHUB_OWNER := alexyarmoshko
GITHUB_REPO := joomla_com_yscbcalendar

# One explicit list of what ships. Never a directory: a packager that discovers its
# own input is how a stray file gets published. The trade is that a new source file
# is silently left out until it is added here - check `make info` after adding one.
PACKAGE_FILES := \
	LICENSE \
	$(MANIFEST) \
	admin/access.xml \
	admin/config.xml \
	admin/language/en-GB/com_yscbcalendar.ini \
	admin/language/en-GB/com_yscbcalendar.sys.ini \
	admin/services/provider.php \
	admin/src/Controller/DisplayController.php \
	admin/src/Extension/YscbcalendarComponent.php \
	admin/src/View/Yscbcalendar/HtmlView.php \
	admin/tmpl/yscbcalendar/default.php \
	media/com_yscbcalendar/css/calendar.css \
	media/com_yscbcalendar/js/calendar.js \
	site/language/en-GB/com_yscbcalendar.ini \
	...
	site/tmpl/calendar/default_month.php \
	site/tmpl/calendar/default_week.php \
	site/tmpl/calendar/login.php
...
LINT_CMD := set -e; \
	...
	node --check media/com_yscbcalendar/js/calendar.js; \
	echo "lint: ok"
...
dist_release:
	...
	@# The macro above degrades to `zip` when the packager is missing. That is
	@# tolerable for a dev package and not for a published one: it would produce a
	@# non-reproducible archive and then stamp a descriptor claiming its checksum.
	@if [ ! -f "$(JZIP)" ]; then \
