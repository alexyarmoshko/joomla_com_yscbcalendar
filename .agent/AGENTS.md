...
```shell
mysql -u root -proot --port=32768 --host=192.168.0.93 db -e "<SQL command>"
```

Example: query list of all tables:

```shell 
mysql -u root -proot --port=32768 --host=192.168.0.93 db -e "SHOW TABLES;"
```

Query table structure:

```shell
mysql -u root -proot --port=32768 --host=192.168.0.93 db -e "DESCRIBE <table_name>;"
```
...
`docker -H tcp://r2d2.dahaus:2375 container logs ddev-test-html-web` gets you web server logs.
`docker -H tcp://r2d2.dahaus:2375 container logs ddev-test-html-db` gets database server logs.
